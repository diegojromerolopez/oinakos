...
func TestSimulation_HelianaBowAttack(t *testing.T) {
	...
	orc := NewCharacter(5, 5, &EntityConfig{Stats: EntityStatsConfig{HealthPoints: IntInterval{Min: 5, Max: 5}}}, 1, false, nil); orc.Name = "orc"; g.World.Characters = append(g.World.Characters, orc)
...
func TestSimulation_OinakosRestrainsTortures(t *testing.T) {
	...
	orc := NewCharacter(1, 1, &EntityConfig{Stats: EntityStatsConfig{HealthPoints: IntInterval{Min: 10, Max: 10}}}, 1, false, nil); orc.Name = "orc"; g.World.Characters = append(g.World.Characters, orc)
...
func TestSimulation_LightHitPain(t *testing.T) {
	...
	target := NewCharacter(1, 1, &EntityConfig{Stats: EntityStatsConfig{HealthPoints: IntInterval{Min: 100, Max: 100}}}, 1, false, nil); target.State.HealthPoints, target.State.Pain = 100, 0
