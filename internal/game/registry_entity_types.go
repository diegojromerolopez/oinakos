...
func (e *InventoryConfigEntry) UnmarshalYAML(v *yaml.Node) error {
	var s string
	if err := v.Decode(&s); err == nil {
		e.ID, e.Count = s, 1
		return nil
	}
	type entry struct { ID string `yaml:"id"`; Count int `yaml:"count"` }
	var ent entry
	if err := v.Decode(&ent); err == nil {
		e.ID, e.Count = ent.ID, ent.Count
...
type EntityConfig struct {
	...
	AssetDir, AudioDir string `yaml:"-"`; StaticImage, BackImage, CorpseImage, CrouchImage, AttackImage, Attack1Image, Attack2Image, HitImage, Hit1Image, Hit2Image, ChoppingImage, DiggingImage, PregnantImage, CookingImage, RestingImage, RestingAdultImage engine.Image `yaml:"-"`
...
type ModelConfig struct {
	ID string; StaticImage, BackImage, CorpseImage, CrouchImage, AttackImage, HitImage, PregnantImage, CookingImage, RestingImage, RestingAdultImage engine.Image
}
