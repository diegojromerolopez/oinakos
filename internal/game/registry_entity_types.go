...
type EntityStatsConfig struct {
	HealthMin IntInterval `yaml:"health_points_min"`; HealthMax IntInterval `yaml:"health_points_max"`
	...
	AttackCooldown IntInterval `yaml:"attack_cooldown"`; AttackRange FloatInterval `yaml:"attack_range"`; ProjectileSpeed FloatInterval `yaml:"projectile_speed"`
...
func (c EntityStatsConfig) Roll() EntityStats {
	return EntityStats{HealthMin: c.HealthMin.Roll(), HealthMax: c.HealthMax.Roll(), HungerMax: c.HungerMax.Roll(), ThirstMax: c.ThirstMax.Roll(), FatigueMax: c.FatigueMax.Roll(), Speed: c.Speed.Roll(), BaseAttack: c.BaseAttack.Roll(), BaseDefense: c.BaseDefense.Roll(), BaseProtection: c.BaseProtection.Roll(), AttackCooldown: c.AttackCooldown.Roll(), AttackRange: c.AttackRange.Roll(), ProjectileSpeed: c.ProjectileSpeed.Roll(), IsMilkable: c.IsMilkable, MilkCooldown: c.MilkCooldown.Roll(), MaxWeight: c.MaxWeight.Roll(), Age: c.Age.Roll()}
}
...
type EntityConfig struct {
	...
	Footprint []FootprintPoint `yaml:"footprint"`; Description string `yaml:"description,omitempty"`; Unique bool `yaml:"unique,omitempty"`; Gender string `yaml:"gender,omitempty"`; IsTransexual bool `yaml:"transexual,omitempty"`; SoundID string `yaml:"-"`; PlayableCharacter string `yaml:"-"`; PrimaryColor string `yaml:"primary_color,omitempty"`; SecondaryColor string `yaml:"secondary_color,omitempty"`; XP int `yaml:"xp,omitempty"`; Group string `yaml:"group,omitempty"`; LeaderID string `yaml:"leader,omitempty"`; MustSurvive bool `yaml:"must_survive,omitempty"`; Playable bool `yaml:"playable,omitempty"`; MaxItems int `yaml:"max_items,omitempty"`; Equipment map[string]string `yaml:"equipment,omitempty"`; Inventory []string `yaml:"inventory,omitempty"`; Denarii int `yaml:"denarii,omitempty"`
	AssetDir, AudioDir string `yaml:"-"`; StaticImage, BackImage, CorpseImage, CrouchImage, AttackImage, Attack1Image, Attack2Image, HitImage, Hit1Image, Hit2Image, ChoppingImage, DiggingImage, PregnantImage engine.Image `yaml:"-"`
...
type ModelConfig struct {
	ID string; StaticImage, BackImage, CorpseImage, CrouchImage, AttackImage, HitImage, PregnantImage engine.Image
}
