...
func (m *MockGraphicsInheritance) DrawImageWithShader(screen engine.Image, img engine.Image, shader engine.Shader, uniforms map[string]any, options *engine.DrawImageOptions) {
}
...
`),
	...
	archReg.Archetypes["man_at_arms_male"] = &EntityConfig{
		ID: "man_at_arms_male",
		Stats: EntityStats{HealthMin: 100},
	}
...
func TestNPC_GenderFallback(t *testing.T) {
	...
	npcReg.LoadAssets(fstest.MapFS{}, &MockGraphicsInheritance{}, archReg, nil, nil)
