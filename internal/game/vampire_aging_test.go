...
func TestIntegration_VampireAging(t *testing.T) {
	...
	p.Name = "Boris"
	p.TemporalState.Age = AgeState{Current: 100.0, Rate: 0.0}
	p.AgeTicks = 100.0 * float64(TicksPerYear)
	p.SyncStats(nil)

	initialAgeTicks := p.AgeTicks
	initialYears := p.TemporalState.Age.Current

	fmt.Printf("Initial Boris Age: %.2f years (Ticks: %.0f), Rate: %.1f\n", 
		p.TemporalState.Age.Current, p.AgeTicks, p.TemporalState.Age.Rate)
	...
	fmt.Printf("After 1 day Boris Age: %.2f years (Ticks: %.0f)\n", 
		p.TemporalState.Age.Current, p.AgeTicks)

	if p.AgeTicks != initialAgeTicks {
		t.Errorf("Vampire aged! Expected ticks %.0f, got %.0f", initialAgeTicks, p.AgeTicks)
	}
	if p.TemporalState.Age.Current != initialYears {
		t.Errorf("Vampire age years changed! Expected %.2f, got %.2f", initialYears, p.TemporalState.Age.Current)
	}
	...
	fmt.Printf("After 1 day Peasant Age: %.5f years (Ticks: %.0f)\n", 
		peasant.TemporalState.Age.Current, peasant.AgeTicks)
