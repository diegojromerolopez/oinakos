...
func TestArchetypeRegistry_CreateLoadJobs(t *testing.T) {
	...
	t.Run("no permit list", func(t *testing.T) {
		...
		// 16 assets per archetype (static, back, corpse, attack, attack1, attack2, hit, hit1, hit2, crouch, chopping, digging, pregnant, cooking, resting, resting_adult)
		if len(jobs) != 16 {
			t.Errorf("expected 16 jobs, got %d", len(jobs))
		}
	})
