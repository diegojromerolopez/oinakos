...
`),
	...
	t.Run("LoneString_DefaultMap", func(t *testing.T) {
		g := NewGame(mockFS, "default1", "", "", NewMockInputManager(), NewMockAudioManager(), false, "0.1-test")
	...
	t.Run("ImplicitExtension_Path", func(t *testing.T) {
		g := NewGame(mockFS, "external/ext1", "", "", NewMockInputManager(), NewMockAudioManager(), false, "0.1-test")
		if g.mapLevel != 5 {
	...
	t.Run("PrioritySearch", func(t *testing.T) {
		...
		g := NewGame(mockFS, "priority.yaml", "", "", NewMockInputManager(), NewMockAudioManager(), false, "0.1-test")
		if g.mapLevel != 1 {
	...
	t.Run("Path_ExternalMap", func(t *testing.T) {
		g := NewGame(mockFS, "external/ext1.yaml", "", "", NewMockInputManager(), NewMockAudioManager(), false, "0.1-test")
	...
	t.Run("NonExistent_LoneString", func(t *testing.T) {
		g := NewGame(mockFS, "nonexistent", "", "", NewMockInputManager(), NewMockAudioManager(), false, "0.1-test")
