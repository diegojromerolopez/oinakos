...
`),
	...
	t.Run("LoneString_DefaultMap", func(t *testing.T) {
		g := NewGame(mockFS, &engine.MockGraphics{}, "default1", "", "", NewMockInputManager(), NewMockAudioManager(), false, "0.1-test")
	...
	t.Run("ImplicitExtension_Path", func(t *testing.T) {
		g := NewGame(mockFS, &engine.MockGraphics{}, "external/ext1", "", "", NewMockInputManager(), NewMockAudioManager(), false, "0.1-test")
		if g.mapLevel != 5 {
	...
	t.Run("PrioritySearch", func(t *testing.T) {
		...
		g := NewGame(mockFS, &engine.MockGraphics{}, "priority.yaml", "", "", NewMockInputManager(), NewMockAudioManager(), false, "0.1-test")
		if g.mapLevel != 1 {
	...
	t.Run("Path_ExternalMap", func(t *testing.T) {
		g := NewGame(mockFS, &engine.MockGraphics{}, "external/ext1.yaml", "", "", NewMockInputManager(), NewMockAudioManager(), false, "0.1-test")
	...
	t.Run("NonExistent_LoneString", func(t *testing.T) {
		g := NewGame(mockFS, &engine.MockGraphics{}, "nonexistent", "", "", NewMockInputManager(), NewMockAudioManager(), false, "0.1-test")
