...
`),
	...
	t.Run("ImplicitExtension_Path", func(t *testing.T) {
		g := NewGame(mockFS, "external/ext1", "", "", NewMockInputManager(), NewMockAudioManager(), false)
		if g.mapLevel != 5 {
	...
	t.Run("PrioritySearch", func(t *testing.T) {
		...
		g := NewGame(mockFS, "priority.yaml", "", "", NewMockInputManager(), NewMockAudioManager(), false)
		if g.mapLevel != 1 {
	...
	t.Run("NonExistent_LoneString", func(t *testing.T) {
		g := NewGame(mockFS, "nonexistent", "", "", NewMockInputManager(), NewMockAudioManager(), false)
