...
const HeadlessVersion = "0.2-alpha-headless"
...
func parseDurationTicks(s string) int {
	...
	for _, m := range matches {
		...
		switch unit {
		case "y":
			total += val * TicksPerYear
		case "m":
			total += val * TicksPerMonth
		case "d":
			total += val * TicksPerDay
		case "h":
			total += val * TicksPerHour
		}
	}
...
func main() {
	...
	case 2:
		updatesPerTick = 1000
	case 3:
		updatesPerTick = 10000
	case 4:
		updatesPerTick = 100000
	case 5:
		updatesPerTick = 500000
	default:
		updatesPerTick = 1
	}
